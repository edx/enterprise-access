...
class EnterpriseSlugReservation(TimeStampedModel):
    ...
    def get_reservation_duration(cls):
        """Get reservation duration from settings or default to 30 minutes."""
        return getattr(settings, 'ENTERPRISE_SLUG_RESERVATION_MINUTES', 30)
    ...
    def reserve_slug(cls, user, slug):
        ...
        If the user already has a reservation, it will be replaced.
        Cleans up expired reservations before attempting to reserve.

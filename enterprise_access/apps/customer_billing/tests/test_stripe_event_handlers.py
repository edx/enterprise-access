...
class TestStripeEventHandler(TestCase):
    ...
    def _create_actual_stripe_event(self, event_type, event_data, **event_attrs):
        """
    ...
    def _create_mock_stripe_event(self, event_type, event_data):
        ...
        mock_event.created = int(_rand_created_at().timestamp())
        mock_event.type = event_type

        mock_event.id = f'evt_test_{event_type.replace(".", "_")}_{_rand_numeric_string()}'
    ...
    def test_subscription_updated_trial_to_active_already_processed(self, mock_license_manager_client):
        ...
        mock_event = self._create_actual_stripe_event(
            "customer.subscription.updated",
            subscription_data,
        )

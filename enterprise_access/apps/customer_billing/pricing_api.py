...
def get_all_active_stripe_prices(
    ...
    """
    Fetch all active recurring Stripe prices and return a serialized list.

    This is the shared cacheable path used by both lookup-key mapping and any
    caller that needs the full active price list.
    """
...
def get_all_active_academy_stripe_prices(
    ...
    """
    Fetch all active academy Stripe prices and return a serialized list.
    """
    ...
    try:
        ...
        logger.error(f'Stripe API error fetching all active academy prices: {exc}')
        raise StripePricingError(f'Failed to fetch all active academy prices: {exc}') from exc
    except Exception as exc:
        logger.error(f'Unexpected error fetching all active academy prices: {exc}')
        raise StripePricingError(f'Unexpected error fetching all active academy prices: {exc}') from exc

...
QUERY = '''
    ...
    -- Last Month’s avg hours of learning per learner (and a percentile rank of
    -- how that compares with over organizations that month). [CHECK]
    ...
    */
    SELECT
        ...
        CASE
            ...
            THEN CONCAT(
                '+',
                CAST(ROUND(oma.avg_hours_per_learner*60 - tma.avg_hours_per_learner*60,1) as VARCHAR),
                ' from last month'
            )
            ELSE CONCAT(
                CAST(ROUND(oma.avg_hours_per_learner*60 - tma.avg_hours_per_learner*60,1) as VARCHAR),
        ...
        CAST(
            ROUND(ROUND(oma.perc_with_sessions - tma.perc_with_sessions,2)*100,0) as VARCHAR
        ) || '%' as perc_with_sessions_delta, --formatting
...
class Command(BaseCommand):
    ...
    Example usage:
    ./manage.py monthly_impact_report
    ./manage.py monthly_impact_report --no-commit
    """
    ...
    def emit_event(self, **kwargs):
        ...
        track_event(external_id, 'edx.bi.enterprise.user.admin.impact_report', kwargs)
        LOGGER.info(

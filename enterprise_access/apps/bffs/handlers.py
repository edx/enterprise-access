...
class BaseLearnerPortalHandler(BaseHandler):
    ...
    def load_and_process(self):
        ...
        try:
            ...
            logger.exception(f"Error loading learner portal handler: {e}")
            self.add_error(
        ...
        for subscription_license in assigned_licenses:
            ...
            if activation_key:
                try:
                    ...
                    logger.exception(f"Error activating license {subscription_license.get('uuid')}: {e}")
                    self.add_error(
    ...
    def check_and_auto_apply_license(self):
        ...
        try:
            ...
            logger.exception(f"Error auto-applying license: {e}")
            self.add_error(
...
class DashboardHandler(BaseLearnerPortalHandler):
    ...
    def load_and_process(self):
        ...
        try:
            ...
            logger.exception(f"Error retrieving enterprise_course_enrollments: {e}")
            self.add_error(

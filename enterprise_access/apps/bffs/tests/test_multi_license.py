...
def _make_license(
    ...
    return {
        'uuid': uuid,
        'status': status,
        'activation_date': _normalize_datetime(activation_date),
        'subscription_plan': {
            'uuid': f'plan-{uuid}',
...
class TestSelectBestLicense(TestCase):
    ...
    def test_iso_datetime_activation_dates_preserve_time_of_day_ordering(self):
        """
        When two licenses activate on the same calendar date, the earliest
        timestamp wins rather than collapsing both values down to the date.
        """
        early_same_day = _make_license('lic-early', '2024-01-01T00:00:00Z', '2026-12-31', 'cat-a')
        late_same_day = _make_license('lic-late', '2024-01-01T18:30:00Z', '2026-12-31', 'cat-a')

        result = self.processor._select_best_license([late_same_day, early_same_day])
        self.assertEqual(result['uuid'], 'lic-early')

    def test_same_day_expiration_keeps_full_datetime_tie_breaker(self):
        """
        Expiration tie-breaks keep full timestamp precision, so the license with
        the later same-day expiration wins when activation dates tie.
        """
        earlier_expiration = _make_license('lic-earlier-exp', '2024-01-01', '2026-12-31T01:00:00Z', 'cat-a')
        later_expiration = _make_license('lic-later-exp', '2024-01-01', '2026-12-31T23:59:59Z', 'cat-a')

        result = self.processor._select_best_license([earlier_expiration, later_expiration])
        self.assertEqual(result['uuid'], 'lic-later-exp')

    def test_none_expiration_date_treated_as_far_past(self):
        """
        A missing expiration_date sorts before a real expiration date when
        activation dates tie, because it is treated as the minimum valid date.
        """
        missing_expiration = _make_license('lic-missing-exp', '2024-01-01', None, 'cat-a')
        real_expiration = _make_license('lic-real-exp', '2024-01-01', '2026-12-31', 'cat-a')

        result = self.processor._select_best_license([missing_expiration, real_expiration])
        self.assertEqual(result['uuid'], 'lic-real-exp')

    def test_unparseable_dates_do_not_raise_and_sort_after_valid_values(self):
        """
        Invalid activation/expiration strings fall back to sentinel datetimes,
        so sorting remains deterministic and valid values still win.
        """
        invalid = _make_license('lic-invalid', 'not-a-date', 'also-not-a-date', 'cat-a')
        valid = _make_license('lic-valid', '2024-01-01', '2026-12-31', 'cat-a')

        result = self.processor._select_best_license([invalid, valid])
        self.assertEqual(result['uuid'], 'lic-valid')
    ...
    def test_multiple_licenses_same_activation_date_different_times_picks_earliest(self):
        """
        When multiple licenses share the same activation date but different times,
        the license with the earliest time-of-day wins. This ensures datetime
        precision is preserved in tie-breaking logic.
        """
        morning = _make_license('lic-morning', '2024-03-15T08:00:00Z', '2026-12-31', 'cat-a')
        afternoon = _make_license('lic-afternoon', '2024-03-15T14:30:00Z', '2026-12-31', 'cat-a')
        evening = _make_license('lic-evening', '2024-03-15T20:45:00Z', '2026-12-31', 'cat-a')

        # Test with different orderings to ensure determinism
        result1 = self.processor._select_best_license([evening, morning, afternoon])
        result2 = self.processor._select_best_license([afternoon, evening, morning])
        result3 = self.processor._select_best_license([morning, afternoon, evening])

        self.assertEqual(result1['uuid'], 'lic-morning')
        self.assertEqual(result2['uuid'], 'lic-morning')
        self.assertEqual(result3['uuid'], 'lic-morning')
...
class TestBuildCatalogIndex(TestCase):
    ...
    def test_groups_licenses_by_catalog(self):
        ...
        self.assertCountEqual([license_obj['uuid'] for license_obj in index['cat-a']], ['lic-1', 'lic-2'])
        self.assertCountEqual([license_obj['uuid'] for license_obj in index['cat-b']], ['lic-3'])
...
class TestMapCoursesToLicenses(TestHandlerContextMixin):
    ...
    def _make_handler(self, activated_licenses):
        """Create a handler with a stubbed context and pre-loaded activated licenses."""
        context = mock.Mock(spec=HandlerContext)
    ...
    def test_intention_without_course_run_key_is_skipped(self):
        """Enrollment intentions without a valid course_run_key are ignored."""
        lic = _make_license('lic-a', '2024-01-01', '2026-12-31', 'cat-a')
        handler = self._make_handler([lic])

        intentions = [
            {'applicable_enterprise_catalog_uuids': ['cat-a']},
            {'course_run_key': None, 'applicable_enterprise_catalog_uuids': ['cat-a']},
            {'course_run_key': '', 'applicable_enterprise_catalog_uuids': ['cat-a']},
            {'course_run_key': 'course-valid', 'applicable_enterprise_catalog_uuids': ['cat-a']},
        ]
        mappings = handler._map_courses_to_licenses(intentions)

        self.assertEqual(mappings, {'course-valid': 'lic-a'})
        self.assertNotIn(None, mappings)
        self.assertNotIn('', mappings)
...
class TestTransformSubscriptionsResult(TestHandlerContextMixin):
    ...
    focusing on the waffle-flag-gated field:
      - licenses_by_catalog (populated only when flag is ON; absent when OFF)
    """
    ...
    def test_flag_off_returns_v1_schema_and_empty_catalog_index(self, _mock_flag):
        """With flag OFF: multi-license-only fields are omitted, but flat list matches main."""
        lic = _make_license('lic-a', '2024-01-01', '2026-12-31', 'cat-a')
        handler = self._make_handler([lic])

        result = handler.transform_subscriptions_result(self._subscriptions_result([lic]))

        self.assertNotIn('license_schema_version', result)
        self.assertNotIn('licenses_by_catalog', result)
    ...
    def test_flag_off_multiple_licenses_preserves_main_flat_list(self, _mock_flag):
        """
        v1 (flag OFF): preserve the legacy flat-list behavior by exposing all
        returned licenses in subscription_licenses, while also surfacing the
        selected singular license in subscription_license.
        """
        lic_a = _make_license('lic-a', '2024-01-01', '2026-12-31', 'cat-a')
        ...
        self.assertCountEqual(
            [license['uuid'] for license in result['subscription_licenses']],
            ['lic-a', 'lic-b', 'lic-c'],
        )
...
class TestScopeSecuredAlgoliaByFlag(TestHandlerContextMixin):
    """Tests for flag-gated multi-license subscription payload fields."""
    ...
    def test_flag_on_returns_populated_catalog_index(self, _mock_flag):
        """With flag ON: licenses_by_catalog is built."""
        lic = _make_license('lic-a', '2024-01-01', '2026-12-31', 'cat-a')
        handler = self._make_handler([lic])
    ...
    @mock.patch('enterprise_access.apps.bffs.handlers.enable_multi_license_entitlements_bff', return_value=True)
    def test_flag_on_ignores_non_current_activated_licenses(self, _mock_flag):
        """Activated licenses with non-current plans are excluded from catalog indexing."""
        non_current = _make_license('lic-a', '2024-01-01', '2026-12-31', 'cat-a', is_current=False)
        handler = self._make_handler([non_current])

        result = handler.transform_subscriptions_result(self._subscriptions_result([non_current]))
    ...
    @mock.patch('enterprise_access.apps.bffs.handlers.enable_multi_license_entitlements_bff', return_value=True)
    @mock.patch.object(BaseLearnerPortalHandler, '_select_best_license', return_value=None)
    def test_flag_on_preserves_subscription_plan_if_best_license_unavailable(self, _mock_best, _mock_flag):
        """Defensive branch: preserve prior subscription_plan if selector returns None unexpectedly."""
        lic = _make_license('lic-a', '2024-01-01', '2026-12-31', 'cat-a')
        handler = self._make_handler([lic])
        ...
        self.assertIsNone(result['subscription_license'])
        self.assertEqual(result['subscription_plan'], lic['subscription_plan'])
...
class TestWaffleFlagRouting(TestHandlerContextMixin):
    ...
    @mock.patch.object(BaseLearnerPortalHandler, '_request_default_enrollment_realizations', return_value={})
    @mock.patch.object(BaseLearnerPortalHandler, '_map_courses_to_licenses', return_value={})
    @mock.patch.object(BaseLearnerPortalHandler, '_map_courses_to_single_license', return_value={})
    def test_no_activated_licenses_skips_realization(
        self, mock_single, mock_multi, mock_request
    ):
        """When there are no activated licenses, enrollment realization is skipped."""
        context = mock.Mock(spec=HandlerContext)
        context.request = self.request
        context.data = {
            'enterprise_customer_user_subsidies': {

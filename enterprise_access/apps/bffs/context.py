...
class HandlerContext:
    ...
    def _initialize_common_context_data(self):
        ...
        if not self.enterprise_customer_slug:
            self._enterprise_customer_slug = self.data.get('enterprise_customer', {}).get('slug')

        if not self.enterprise_customer_uuid:
            self._enterprise_customer_uuid = self.data.get('enterprise_customer', {}).get('uuid')
    ...
    def _initialize_enterprise_customer_users(self):
        ...
        enterprise_customer_users = [
            self._transform_enterprise_customer_user(enterprise_customer_user)
            for enterprise_customer_user in enterprise_customer_users_data.get('results', [])
        ]
        ...
        if (
            not enterprise_customer_user_for_requested_customer
            and has_enterprise_customer_slug_or_uuid
        ...
        self.data = {
            **self.data,
            ...
            'staff_enterprise_customer': self._transform_enterprise_customer(staff_enterprise_customer),
        }
    ...
    def _enterprise_customer_matches_slug_or_uuid(self, enterprise_customer):
        ...
        """
        return (
            enterprise_customer.get('slug') == self.enterprise_customer_slug
            or enterprise_customer.get('uuid') == self.enterprise_customer_uuid
        )

    def _transform_enterprise_customer_user(self, enterprise_customer_user):
        """
        Transform the enterprise customer user data.
        ...
        return {
            **enterprise_customer_user,
            'enterprise_customer': self._transform_enterprise_customer(enterprise_customer)
        }

    def _transform_enterprise_customer(self, enterprise_customer):
        """
        ...
        disable_search = bool(
            not enterprise_customer.get("enable_integrated_customer_learner_portal_search", False)
